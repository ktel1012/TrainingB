...
{
    public class MN_Find3dDUOI(ChromeDriver driver) : FindBase(driver)
    {
        private const string REGION_KEY = "Southern_3d_Alt";
        private const string URL_KEY = "MN_3d_Duoi";
                ...
                try
                {
                    ...
                    foreach (var l3 in l2)
                    {
                        ...
                        foreach (var txt2 in txt.Split('\n'))
                        {
                            ...
                            if (ls.Count >= 4)
                            {
                                if (!TryParseInt(ls[0], out int val0, "MN_Find3dDUOI") ||
                                    !TryParseInt(ls[1], out int val1, "MN_Find3dDUOI") ||
                                    !TryParseInt(ls[2], out int val2, "MN_Find3dDUOI"))
                                {
                                    ls = new List<string>();
                                    continue;
                                }
                    ...
                    return ProcessResults(dic, threshold, "3dDuoi");
                }
                catch (Exception ex)
                {
                    Logger.Error("Error in MN_Find3dDUOI.GetChangeList", ex);
                    throw;
                }
            }, "MN_Find3dDUOI.GetChangeList");
        }
    }
}
