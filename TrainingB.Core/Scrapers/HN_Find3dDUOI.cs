...
{
    public class HN_Find3dDUOI(ChromeDriver driver) : FindBase(driver)
    {
        private const string REGION_KEY = "Northern_3d";
        private const string URL_KEY = "HN_3d_Duoi";
        ...
        public override string GetChangeList()
        {
            return ExecuteWithRetry(() =>
            {
                try
                {
                    ...
                    foreach (var l3 in l2)
                    {
                        ...
                        foreach (var txt2 in txt.Split('\n'))
                        {
                            ...
                            if (ls.Count >= 2)
                            {
                                if (!TryParseInt(ls[1], out int sum, "HN_Find3dDUOI"))
                                {
                                    ls = new List<string>();
                                    continue;
                                }
                ...
                {
                    Logger.Error("Error in HN_Find3dDUOI.GetChangeList", ex);
                    throw;
                }
            }, "HN_Find3dDUOI.GetChangeList");
        }
    }
}
